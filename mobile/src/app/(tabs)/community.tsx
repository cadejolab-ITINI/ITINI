...
export default function CommunityScreen() {
  ...
  const publish = async () => {
    if (!profile || !defaultDestination || !visitedPlace.trim() || !text.trim()) return;
    const post: CommunityPost = {
      ...
      destinationName: visitedPlace.trim(),
      rating,
      text: text.trim(),
      likes: 0,
      liked: false,
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.composer}>
          <View style={styles.composerHeader}>
            <Text style={styles.composerTitle}>Compartir mi experiencia</Text>
          ...
          <TextInput value={visitedPlace} onChangeText={setVisitedPlace} placeholder="Nombre del lugar visitado" placeholderTextColor="#9AB4D0" style={styles.placeInput} accessibilityLabel="Nombre del lugar visitado" />

          <TextInput value={text} onChangeText={setText} placeholder="¿Cómo estuvo el sendero? ¿Recomendaciones de horario o clima?" placeholderTextColor="#9AB4D0" multiline style={styles.reviewInput} />
          {photoUri && <><Image source={{ uri: photoUri }} style={styles.photoPreview} /><Pressable onPress={() => setPhotoUri(null)} accessibilityRole="button" accessibilityLabel="Quitar foto" style={styles.removePhoto}><MaterialCommunityIcons name="trash-can-outline" size={16} color="#F0444D" /><Text style={styles.removePhotoText}>Quitar foto</Text></Pressable></>}
        ...
        {communityPosts.map((post) => (
          <View style={styles.post} key={post.id}>
            <View style={styles.postHeader}>
              ...
              <View style={styles.postIdentity}><Text style={styles.postAuthor}>{post.author}</Text><Text style={styles.postRole}>{post.role}</Text></View>
...
const styles = StyleSheet.create({
  ...
  placeInput: { minHeight: 38, borderRadius: 11, borderWidth: 1, borderColor: '#263852', backgroundColor: '#03091A', color: '#FFFFFF', paddingHorizontal: 11, fontFamily: font.bold, fontSize: 11, outlineStyle: 'none' } as never,
  ...
  removePhoto: { alignSelf: 'flex-start', minHeight: 30, paddingHorizontal: 10, borderRadius: 9, borderWidth: 1, borderColor: 'rgba(240,68,77,0.45)', backgroundColor: 'rgba(240,68,77,0.1)', flexDirection: 'row', alignItems: 'center', gap: 5 },
  removePhotoText: { color: '#F0444D', fontFamily: font.bold, fontSize: 10 },
