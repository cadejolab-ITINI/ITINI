    ...
    <Screen>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.composer}>
          ...
          <Pressable onPress={() => setDestinationOpen((current) => !current)} style={styles.select}>
            <Text style={styles.selectText}>{selectedDestination?.name ?? 'Seleccioná un destino'}</Text>
            <MaterialCommunityIcons name="chevron-down" size={22} color="#FFFFFF" />
          </Pressable>
          {destinationOpen && <View style={styles.destinationMenu}>{destinations.map((destination) => <Pressable key={destination.id} onPress={() => { setDestinationId(destination.id); setDestinationOpen(false); }} style={styles.destinationOption}><Text style={styles.destinationOptionText}>{destination.name}</Text></Pressable>)}</View>}
