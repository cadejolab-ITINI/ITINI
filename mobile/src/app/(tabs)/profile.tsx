...
export default function ProfileScreen() {
  const { profile, updateProfile, stats, settings } = useAppData();
  ...
  return (
    <Screen style={settings.lightMode ? styles.screenLight : undefined}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        <Text style={[styles.title, settings.lightMode && styles.titleLight]}>Perfil de Usuario</Text>
        <View style={[styles.profileCard, settings.lightMode && styles.cardLight]}>
          <View style={styles.avatar}><Text style={styles.avatarText}>{profile.avatar}</Text></View>
          <Text style={[styles.name, settings.lightMode && styles.darkText]}>{profile.name}</Text>
          <Text style={styles.username}>{profile.username}</Text>
          <Text style={[styles.bio, settings.lightMode && styles.darkText]}>{profile.bio}</Text>
          ...
          {editing && <View style={styles.form}><TextInput value={name} onChangeText={setName} style={[styles.input, settings.lightMode && styles.fieldLight]} placeholder="Nombre" placeholderTextColor="#7488A2" /><TextInput value={bio} onChangeText={setBio} style={[styles.input, styles.bioInput, settings.lightMode && styles.fieldLight]} multiline placeholder="Biografía" placeholderTextColor="#7488A2" /><Pressable onPress={save} style={styles.saveButton}><Text style={styles.saveText}>Guardar cambios</Text></Pressable></View>}
        </View>

        <View style={styles.stats}>
          <Stat value={String(stats.visits)} label="Visitas registradas" color="#20D5A0" light={settings.lightMode} />
          <Stat value="—" label="Verificaciones pendientes" color="#24BDF4" light={settings.lightMode} />
          <Stat value={String(stats.reviews)} label="Reseñas propias nuevas" color="#FF7A2A" light={settings.lightMode} />
        </View>
      </ScrollView>
    </Screen>
  );
...
function Stat({ value, label, color, light }: { value: string; label: string; color: string; light: boolean }) {
  return <View style={[styles.stat, light && styles.cardLight]}><Text style={[styles.statValue, { color }]}>{value}</Text><Text style={styles.statLabel}>{label}</Text></View>;
}
...
const styles = StyleSheet.create({
  ...
  fieldLight: { borderColor: '#C7D5E3', backgroundColor: '#F1F5F9', color: '#172B43' },
