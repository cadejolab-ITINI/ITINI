...
const settingKeys = { offlineMode: 'offline_mode', reducedData: 'reduced_data' } as const;
export async function getSettings(db: SQLiteDatabase): Promise<Settings> {
  ...
  return { offlineMode: rows.some(row => row.key === 'offline_mode' && row.value === 1), reducedData: rows.some(row => row.key === 'reduced_data' && row.value === 1) };
}
